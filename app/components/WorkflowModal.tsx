  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Section 1: Manual input — user types the prompt (also filled when Generate Prompt is used in AI-written) */}
        <div
          style={{
            padding: "10px 12px",
        ...
        >
          ...
          <div>
            <textarea
              ...
              placeholder="Describe the background in your own words (or use AI-written prompt: Generate Prompt fills this too)"
              rows={2}
              disabled={generationMode !== "manual"}
              style={{
                width: "100%",
                ...
                resize: "vertical",
                fontFamily: "inherit",
                ...(generationMode !== "manual" ? { background: "var(--p-color-bg-surface-secondary, #f6f6f7)", cursor: "not-allowed" } : {}),
              }}
            />
          </div>
        </div>
        {/* Section 2: AI-written prompt — generate prompt from product + mood/style/env (fills same prompt used by manual); then send as manual_prompt */}
        <div
          style={{
            padding: "10px 12px",
            borderBottom: "1px solid var(--p-color-border-secondary, #e1e3e5)",
        ...
        >
          ...
          <div style={{ display: "flex", alignItems: "flex-start", gap: "8px" }}>
            <textarea
              value={prompt}
              onChange={(e) => setPrompt(e.target.value)}
              placeholder="Select mood, style & environment below and click Generate Prompt (also fills Manual input above)"
              rows={2}
              disabled={generationMode !== "aiPrompt"}
              style={{
                flex: 1,
                minWidth: 0,
                padding: "8px 10px",
                ...
                resize: "vertical",
                fontFamily: "inherit",
                ...(generationMode !== "aiPrompt" ? { background: "var(--p-color-bg-surface-secondary, #f6f6f7)", cursor: "not-allowed" } : {}),
              }}
            />
            <button
              type="button"
              onClick={handleExtractPrompt}
              disabled={extractPromptLoading || generationMode !== "aiPrompt"}
              style={{
                padding: "8px 14px",
                borderRadius: "8px",
                border: "none",
                background: extractPromptLoading || generationMode !== "aiPrompt" ? "#9ca3af" : "var(--p-color-bg-fill-info, #2c6ecb)",
                color: "#fff",
                fontWeight: 600,
                cursor: extractPromptLoading || generationMode !== "aiPrompt" ? "not-allowed" : "pointer",
                fontSize: "12px",
                flexShrink: 0,
              }}
            >
...
export function WorkflowModal({
  ...
  const handleConfirmGoPrevious = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok && productId?.trim()) {
        loadShortFetcher.load(`${SHORTS_API}?productId=${encodeURIComponent(productId.trim())}&_=${Date.now()}`);
      } else if (res.ok) {
        refetchShort();
      }
    } catch {
  ...
  }, [confirmGoPrevious, refetchShort, productId, loadShortFetcher]);
                        ...
                        <div style={{ flex: "1 1 200px", minWidth: 0 }}>
                          ...
                          <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                            ...
                            <button
                              type="button"
                              title="Test voice"
                              disabled={!selectedVoiceId || !shortInfo?.userId || testAudioLoading}
                              onClick={async () => {
                              ...
                              style={{
                                ...
                                borderRadius: "8px",
                                border: "1px solid var(--p-color-border-secondary, #e1e3e5)",
                                background: testAudioLoading || !selectedVoiceId ? "#f1f2f4" : "#fff",
                                color: "var(--p-color-text-primary, #202223)",
                                cursor: testAudioLoading || !selectedVoiceId ? "not-allowed" : "pointer",
                                display: "flex",
                                alignItems: "center",
                                justifyContent: "center",
                              }}
                            ...
                            >
                              {testAudioLoading ? (
                                <>
                                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden style={{ animation: "spin 0.8s linear infinite" }}>
                                    <path d="M21 12a9 9 0 1 1-6.219-8.56" />
                                  </svg>
                                  <span style={{ fontSize: "12px", marginLeft: "6px", whiteSpace: "nowrap" }}>Loading…</span>
                                </>
                              ) : (
                            ...
                            {!audioGenerated ? (
                              <div style={{ display: "flex", flexDirection: "row", alignItems: "center", gap: "12px", flexWrap: "wrap" }}>
                                ...
                                <button
                                  ...
                                  style={{ padding: "10px 20px", borderRadius: "8px", border: "none", background: "var(--p-color-bg-fill-info, #2c6ecb)", color: "#fff", fontWeight: 600, cursor: "pointer", fontSize: "14px" }}
                                >
                                  Next step →
                                </button>
                              </div>
            ...
            <div style={{ padding: "24px", overflow: "auto", flex: 1 }}>
              ...
              <div style={{ display: activeTab === "scene3" ? "block" : "none" }} key={`scene3-${scene3ResetKey}`}>
                <Scene3Content
                  ...
                  allScenesVideosGenerated={scene1Complete && scene2Complete && scene3Complete}
                />
              </div>
            </div>
            )}
...
const AUDIO_TEST_AUDIO_API = "/app/api/audio/test-audio";
      ...
      <div style={navBarStyle}>
        ...
        <div>
          ...
          {step === 3 && onGoToScene2 && (
            <button
              type="button"
              onClick={onGoToScene2}
              disabled={!(dbSceneVideoUrl ?? sceneVideo)}
              style={{
                ...navBtnPrimaryStyle,
                ...(!(dbSceneVideoUrl ?? sceneVideo) ? { background: "#9ca3af", cursor: "not-allowed" } : {}),
              }}
            >
              Next step →
            </button>
            ...
            {bgRemoved && (
              <div style={{ display: "flex", flexDirection: "column", gap: "4px", alignItems: "flex-start" }}>
                ...
                <button
                  ...
                  style={{ ...navBtnPrimaryStyle, marginTop: "8px" }}
                >
                  Next step →
                </button>
              </div>
...
function Scene2Content({
  ...
  const firstId = productImagesProp[0]?.id ?? "s1";
  const step = scene2StatusToStep(sceneStatus);
...
function Scene3Content({
  ...
  getNextStatus?: (current: string) => string | null;
  onGoToAudio?: () => void;
  /** When true, the step 3 "Next step" to audio is enabled (all scene 1/2/3 videos generated). */
  allScenesVideosGenerated?: boolean;
}) {
  const firstId = productImagesProp[0]?.id ?? "s1";
  const stepFromStatus = scene13StatusToStep(sceneStatus);
  const [step, setStep] = useState(stepFromStatus);
        ...
        <div>
          ...
          {step === 3 && onGoToAudio && (
            <button
              type="button"
              onClick={onGoToAudio}
              disabled={!allScenesVideosGenerated}
              style={{
                ...navBtnPrimaryStyle,
                ...(!allScenesVideosGenerated ? { background: "#9ca3af", cursor: "not-allowed" } : {}),
              }}
            >
              Next step →
            </button>
